...
namespace baselib {
    ...
    sizeof(intptr_t) * 2)mpmc_node_queue {
    public:
    ...
    T *try_pop_front() {
        ...
        if (PLATFORM_LLSC_NATIVE_SUPPORT) {
            ...
            T * front = node;
            if (!m_FrontPair.ptr.compare_exchange_strong(front, reinterpret_cast<T *>(value | 1),
            ...
            if (!m_Back.compare_exchange_strong(front, 0, memory_order_acquire,
                                                memory_order_relaxed)) {
            ...
            } while (!m_Front.compare_exchange_strong(front, value, memory_order_acquire,
                                                      memory_order_relaxed));
            return node;
            ...
            if (!m_Front.compare_exchange_strong(front, value, memory_order_acquire,
                                                 memory_order_relaxed))
                return 0;
            ...
            // - check if node is back == retrigger new back
            value.ptr = node;
            if (!m_Back.compare_exchange_strong(value.ptr, 0, memory_order_acquire,
                                                memory_order_relaxed)) {
                // Back progressed or node is incomplete, restore access and return 0
    ...
    static_assert(std::is_base_of<baselib::mpmc_node, T>::value,
    "Node class/struct used with baselib::mpmc_node_queue must derive from baselib::mpmc_node.");
};
}
}
