...
namespace baselib {
    ...
    class tlsf_allocator
            : protected detail::tlsf_allocator<min_size, max_size, linear_subdivisions, Allocator> {
        using Impl = detail::tlsf_allocator<min_size, max_size, linear_subdivisions, Allocator>;

        static_assert(min_size
        <= max_size, "min_size > max_size");
        static_assert(min_size
        >= linear_subdivisions, "min_size < linear_subdivisions");
        static_assert(max_size
        <=

        std::numeric_limits<size_t>::max()

        / 2 + 1, "max_size > std::numeric_limits<size_t>::max() / 2 + 1");

        static_assert(baselib::Algorithm::IsPowerOfTwo(linear_subdivisions),

        "linear_subdivisions != pow2");
    ...
    public:
        ...
        void *try_reallocate(void *ptr, size_t old_size, size_t new_size) {
            return owns(nullptr, new_size) ? Impl::try_reallocate(ptr, old_size, new_size)
                                           : nullptr;
        }
