...
typedef struct Baselib_HighCapacitySemaphore {
    ...
    char _cachelineSpacer0[
            PLATFORM_CACHE_LINE_SIZE - sizeof(int64_t) - sizeof(Baselib_SystemSemaphore_Handle)];
...
BASELIB_STATIC_ASSERT((offsetof(Baselib_HighCapacitySemaphore, count) + PLATFORM_CACHE_LINE_SIZE) ==
                      offsetof(Baselib_HighCapacitySemaphore, _systemSemaphoreData),
                      "count and internalData must not share cacheline");
...
if (
OPTIMIZER_LIKELY(Baselib_SystemSemaphore_TryTimedAcquire(semaphore->handle, timeoutInMilliseconds)
))
return true;
...
while (!
Baselib_SystemSemaphore_TryAcquire(semaphore
->handle));
return true;
}
...
<= (int64_t)UINT32_MAX);
Baselib_SystemSemaphore_Release(semaphore
->handle, (uint32_t)threadsToWakeup);
return;
}
