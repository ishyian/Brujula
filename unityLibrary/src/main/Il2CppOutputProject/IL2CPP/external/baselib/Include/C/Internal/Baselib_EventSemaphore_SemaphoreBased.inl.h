...
typedef struct Baselib_EventSemaphore {
    ...
    char _cachelineSpacer0[PLATFORM_CACHE_LINE_SIZE - 2 * sizeof(Baselib_SystemSemaphore_Handle) -
                           sizeof(Detail_Baselib_EventSemaphore_State)];
...
BASELIB_STATIC_ASSERT((offsetof(Baselib_EventSemaphore, state) + PLATFORM_CACHE_LINE_SIZE) ==
                      offsetof(Baselib_EventSemaphore, _systemSemaphoreDataSemaphore),
                      "state and _systemSemaphoreDataSemaphore must not share cacheline");

BASELIB_STATIC_ASSERT((offsetof(Baselib_EventSemaphore, _systemSemaphoreDataSemaphore) +
                       PLATFORM_CACHE_LINE_SIZE) ==
                      offsetof(Baselib_EventSemaphore, _systemSemaphoreDataInProgressSemaphore),
                      "_systemSemaphoreDataSemaphore and _systemSemaphoreDataInProgressSemaphore must not share cacheline");
...
BASELIB_API void Detail_Baselib_EventSemaphore_SemaphoreBased_AcquireNonSet(int32_t
initialNumWaitingForSetAndStateFlags,
Baselib_EventSemaphore *semaphore
);
COMPILER_WARN_UNUSED_RESULT
        BASELIB_API
bool Detail_Baselib_EventSemaphore_SemaphoreBased_TryTimedAcquireNonSet(int32_t
initialNumWaitingForSetAndStateFlags,
Baselib_EventSemaphore *semaphore, uint32_t
...
static FORCE_INLINE bool Detail_Baselib_EventSemaphore_IsSet(int32_t
numWaitingForSetAndStateFlags)
{
return (
numWaitingForSetAndStateFlags &Detail_Baselib_EventSemaphore_SetFlag
) ? true : false;
}

static FORCE_INLINE bool Detail_Baselib_EventSemaphore_IsSetInProgress(int32_t
numWaitingForSetAndStateFlags)
{
return (
numWaitingForSetAndStateFlags &Detail_Baselib_EventSemaphore_SetInProgressFlag
) ? true : false;
}

static FORCE_INLINE int32_t
Detail_Baselib_EventSemaphore_GetWaitingForSetCount(int32_t
numWaitingForSetAndStateFlags)
{
return
numWaitingForSetAndStateFlags &Detail_Baselib_EventSemaphore_NumWaitingForSetMask;
}

// Changes WaitingForSet count without affecting state flags
static FORCE_INLINE int32_t
Detail_Baselib_EventSemaphore_SetWaitingForSetCount(int32_t
currentNumWaitingForSetAndStateFlags,
int32_t newNumWaitingForSet
)
{
return newNumWaitingForSet | (
currentNumWaitingForSetAndStateFlags &(~Detail_Baselib_EventSemaphore_NumWaitingForSetMask)
);
}

BASELIB_INLINE_API Baselib_EventSemaphore

Baselib_EventSemaphore_Create(void) {
    Baselib_EventSemaphore semaphore = {{{0, 0}},
                                        {0},
                                        {0},
                                        {0},
                                        {0},
    ...
    semaphore.setSemaphore = Baselib_SystemSemaphore_CreateInplace(
            semaphore._systemSemaphoreDataSemaphore);
    semaphore.setInProgressSemaphore = Baselib_SystemSemaphore_CreateInplace(
            semaphore._systemSemaphoreDataInProgressSemaphore);
    return semaphore;
}
...
bool Baselib_EventSemaphore_TryAcquire(Baselib_EventSemaphore * semaphore) {
    const int32_t numWaitingForSetAndStateFlags = Baselib_atomic_load_32_acquire(
...
BASELIB_INLINE_API void Baselib_EventSemaphore_Acquire(Baselib_EventSemaphore * semaphore) {
    ...
    if (!Detail_Baselib_EventSemaphore_IsSet(numWaitingForSetAndStateFlags))
        Detail_Baselib_EventSemaphore_SemaphoreBased_AcquireNonSet(numWaitingForSetAndStateFlags,
                                                                   semaphore);
}

COMPILER_WARN_UNUSED_RESULT
        BASELIB_INLINE_API
bool Baselib_EventSemaphore_TryTimedAcquire(Baselib_EventSemaphore * semaphore,
...
const int32_t numWaitingForSetAndStateFlags = Baselib_atomic_load_32_acquire(
        &semaphore->state.parts.numWaitingForSetAndStateFlags);
if (!
Detail_Baselib_EventSemaphore_IsSet(numWaitingForSetAndStateFlags)
)
return
Detail_Baselib_EventSemaphore_SemaphoreBased_TryTimedAcquireNonSet(numWaitingForSetAndStateFlags, semaphore, timeoutInMilliseconds
);
return true;
}
...
BASELIB_INLINE_API void Baselib_EventSemaphore_Reset(Baselib_EventSemaphore * semaphore) {
    ...
    int32_t numWaitingForSetAndStateFlags = Baselib_atomic_load_32_relaxed(
            &semaphore->state.parts.numWaitingForSetAndStateFlags);
    do {
...
BASELIB_INLINE_API void Baselib_EventSemaphore_Set(Baselib_EventSemaphore * semaphore) {
    ...
    do {
        numWaitingForSetAndStateFlagsSet =
                numWaitingForSetAndStateFlags | Detail_Baselib_EventSemaphore_SetFlag;
        numWaitingForSet = Detail_Baselib_EventSemaphore_GetWaitingForSetCount(
                numWaitingForSetAndStateFlags);
        BaselibAssert(numWaitingForSet >= 0,
                      "There needs to be always a non-negative amount of threads waiting for Set");
        if (numWaitingForSet)
    ...
    if (!Detail_Baselib_EventSemaphore_IsSetInProgress(numWaitingForSetAndStateFlags) &&
        numWaitingForSet)
        Baselib_SystemSemaphore_Release(semaphore->setSemaphore, numWaitingForSet);
}

BASELIB_INLINE_API void
Baselib_EventSemaphore_ResetAndReleaseWaitingThreads(Baselib_EventSemaphore * semaphore) {
