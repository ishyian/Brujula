...
namespace baselib {
    ...
    inline bool ConditionVariable::TimedWait(const timeout_ms timeoutInMilliseconds) {
        ...
        do {
            ...
            while (waiters > 0) {
                if (m_Data.waiters.compare_exchange_weak(waiters, waiters - 1, memory_order_relaxed,
    ...
    inline void ConditionVariable::Notify(uint16_t count) {
        ...
        do {
            ...
            if (m_Data.waiters.compare_exchange_weak(waitingThreads,
                                                     waitingThreads - threadsToWakeup,
                                                     memory_order_relaxed, memory_order_relaxed)) {
                m_Data.wakeups.fetch_add(threadsToWakeup, memory_order_release);
                Baselib_SystemFutex_Notify(&m_Data.wakeups.obj, threadsToWakeup,
