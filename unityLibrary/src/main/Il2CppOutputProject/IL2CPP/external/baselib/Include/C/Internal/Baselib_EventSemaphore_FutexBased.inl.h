...
BASELIB_STATIC_ASSERT(sizeof(Baselib_EventSemaphore) == PLATFORM_CACHE_LINE_SIZE,
                      "Baselib_EventSemaphore size should match cacheline size (64bytes)");
...
Detail_Baselib_EventSemaphore_Generation(int32_t
state)
{
return
state &Detail_Baselib_EventSemaphore_GenMask;
}
...
static FORCE_INLINE uint32_t
Detail_Baselib_EventSemaphore_TransitionFrom_ResetNoWaitingThreadsState_To_ResetState(Baselib_EventSemaphore
* semaphore)
{
int32_t state = Baselib_atomic_load_32_acquire(&semaphore->state);
const int32_t resetState =
        Detail_Baselib_EventSemaphore_Generation(state) | Detail_Baselib_EventSemaphore_Reset;
...
BASELIB_INLINE_API void Baselib_EventSemaphore_Acquire(Baselib_EventSemaphore * semaphore) {
    const int32_t state = Detail_Baselib_EventSemaphore_TransitionFrom_ResetNoWaitingThreadsState_To_ResetState(
            semaphore);
...
const int32_t state = Detail_Baselib_EventSemaphore_TransitionFrom_ResetNoWaitingThreadsState_To_ResetState(
        semaphore);
if (
state &Detail_Baselib_EventSemaphore_Set
)
return true;
...
// Set-Gen(X), Set-Gen(X+n), ResetNoWaitingThreads-Gen(X+n) or Reset(X+n). (with n>0)
if (state !=
Baselib_atomic_load_32_relaxed(&semaphore
->state))
return true;
...
BASELIB_INLINE_API void Baselib_EventSemaphore_Reset(Baselib_EventSemaphore * semaphore) {
    ...
    const int32_t setState =
            Detail_Baselib_EventSemaphore_Generation(state) | Detail_Baselib_EventSemaphore_Set;
    while (state == setState) {
        const int32_t resetNoWaitingThreadsState = Detail_Baselib_EventSemaphore_Generation(state);
        if (Baselib_atomic_compare_exchange_weak_32_release_relaxed(&semaphore->state, &state,
                                                                    resetNoWaitingThreadsState))
            return;
    }
...
BASELIB_INLINE_API void Baselib_EventSemaphore_Set(Baselib_EventSemaphore * semaphore) {
    ...
    while (state == resetState) {
        const int32_t nextGenSetState = Detail_Baselib_EventSemaphore_Generation(state + 1) |
                                        Detail_Baselib_EventSemaphore_Set;
        if (Baselib_atomic_compare_exchange_weak_32_release_relaxed(&semaphore->state, &state,
                                                                    nextGenSetState)) {
            Baselib_SystemFutex_Notify(&semaphore->state, UINT32_MAX,
...
Baselib_EventSemaphore_ResetAndReleaseWaitingThreads(Baselib_EventSemaphore * semaphore) {
    ...
    // If this is not the case however, we do exactly that, increase the generation & wake all threads.
    while (state == resetState) {
        const int32_t nextGenPendingResetState = Detail_Baselib_EventSemaphore_Generation(
                state + 1);
        if (Baselib_atomic_compare_exchange_weak_32_relaxed_relaxed(&semaphore->state, &state,
                                                                    nextGenPendingResetState)) {
            Baselib_SystemFutex_Notify(&semaphore->state, UINT32_MAX,
                                       Baselib_WakeupFallbackStrategy_All);
            return;
        }
