    ...
    namespace detail {
        ...
        class tlsf_block_allocator {
            ...
            bool ExpandCapacity(size_t size, size_t block_size, Allocator &allocator) {
                ...
                uintptr_t segmentMemory = reinterpret_cast<uintptr_t>(allocator.allocate(
                        segment_size));
                if (segmentMemory == 0)
                    return false;
                ...
                baselib::atomic_fetch_add_explicit(m_Capacity, block_size * block_count,
        ...
        public:
            ...
            tlsf_block_allocator()
                    : m_CapacityLock(), m_Capacity(0), m_FreeBlocks(), m_Segments(nullptr) {}
            ...
            bool deallocate(void *ptr_first, void *ptr_last) {
                m_FreeBlocks.push_back(reinterpret_cast<baselib::mpmc_node *>(ptr_first),
                                       reinterpret_cast<baselib::mpmc_node *>(ptr_last));
                return true;
            }
            ...
            bool reserve(size_t size, size_t capacity, Allocator &allocator) {
                ...
                m_CapacityLock.AcquireScoped([&] {
                    result = capacity > m_Capacity ? ExpandCapacity(capacity - m_Capacity, size,
                                                                    allocator) : true;
                });
                return result;
            }
            ...
            bool increase_capacity(size_t size, Allocator &allocator) {
                ...
                m_CapacityLock.AcquireScoped([&] {
                    if (m_FreeBlocks.empty())
                        result = ExpandCapacity(m_Capacity == 0 ? size : m_Capacity, size,
                                                allocator);
                });
                return result;
            }
            ...
            optimal_size(const size_t size) {
        ...
        public:
            static constexpr uint32_t
            alignment = BaseAllocator::alignment;
            ...
            void *allocate(size_t size) {
                ...
                do {
                    ...
                    if (OPTIMIZER_LIKELY(p = allocator.allocate()))
                        return p;
                    if (!allocator.increase_capacity(AllocatorSize(size),
                                                     static_cast<BaseAllocator &>(*this)))
                        return nullptr;
                } while (true);
            }
            ...
            bool batch_deallocate(void *ptr_first, void *ptr_last, size_t size) {
                return ((ptr_first == nullptr) || (ptr_last == nullptr)) ? false : getAllocator(
                        size).deallocate(ptr_first, ptr_last);
            }
            ...
            bool reserve(size_t size, size_t capacity) {
                return getAllocator(size).reserve(AllocatorSize(size), capacity,
                                                  static_cast<BaseAllocator &>(*this));
            }
        ...
        private:
            struct CompileTime {
                ...
                Log2Base(size_t
                value, size_t offset) {
                    return (value > 1) ? Log2Base(value >> (size_t)1, offset + 1) : offset;
                }
                static constexpr size_t
                Log2Base(size_t
                value) { return Log2Base(value, 0); }
                static constexpr size_t
                Max(size_t
                a, size_t b) { return a > b ? a : b; }
            };

            static constexpr size_t
            m_MinSize = CompileTime::Max(min_size, CompileTime::Max(
                    CompileTime::Max(sizeof(void *), linear_subdivisions), alignment));
            static constexpr size_t
            m_MinSizePow2 = baselib::Algorithm::CeilPowerOfTwo(m_MinSize);
            static constexpr size_t
            m_MaxSizePow2 = baselib::Algorithm::CeilPowerOfTwo(
                    CompileTime::Max(max_size, m_MinSize));
            static constexpr size_t
            m_MinSizeMask = static_cast<size_t>(1) << CompileTime::Log2Base(m_MinSizePow2 - 1);
            static constexpr size_t
            m_AllocatorCount =
            (CompileTime::Log2Base(m_MaxSizePow2) - CompileTime::Log2Base(m_MinSizePow2)) + 1;
            static constexpr size_t
            m_AllocatorBaseOffsetLog2 = CompileTime::Log2Base(m_MinSizePow2) - 1;
            static constexpr size_t
            m_LinearSubdivisionsLog2 = CompileTime::Log2Base(linear_subdivisions);

            static constexpr size_t
            AllocatorSizeLog2(size_t
            size) { return baselib::Algorithm::HighestBitNonZero(size | m_MinSizeMask); }
            static constexpr size_t
            LinearAllocatorSizeLog2(size_t
            size,
            size_t sizeLog2
            ) {
                return (size & ((size_t)
                1 << sizeLog2) -1) >> (sizeLog2 - m_LinearSubdivisionsLog2);
            }

            template<int value = ((m_AllocatorCount == 1 && linear_subdivisions == 1) ? 1
                                                                                      : 2), typename std::enable_if<(
                    value == 1), int>::type = 0>
            static constexpr FORCE_INLINE

            size_t AllocatorSize(size_t size) {
                return m_MinSizePow2;
            }

            template<int value = ((m_AllocatorCount != 1 && linear_subdivisions == 1) ? 3
            ...
            size_t AllocatorSize(size_t size) {
                return (size_t)
                1 << (AllocatorSizeLog2(size - 1) + 1);
            }

            template<int value = (linear_subdivisions == 1) ? 0
                                                            : 1, typename std::enable_if<(value), int>::type = 0>
            static FORCE_INLINE size_t
            AllocatorSize(size_t
            size)
            {
                const size_t subDivSize = ((size_t)
                1 << baselib::Algorithm::HighestBitNonZero(size)) >> m_LinearSubdivisionsLog2;
            ...
            template<int value = ((m_AllocatorCount == 1 && linear_subdivisions != 1) ? 5
            ...
            template<int value = ((m_AllocatorCount != 1 && linear_subdivisions != 1) ? 7
                                                                                      : 8), typename std::enable_if<(
                    value == 7), int>::type = 0>
            BlockAllocator &getAllocator(size_t size) {
                --size;
                const size_t sizeLog2 = AllocatorSizeLog2(size);
                return m_Allocators[sizeLog2 - m_AllocatorBaseOffsetLog2][LinearAllocatorSizeLog2(
                        size, sizeLog2)];
            }
            ...
            struct has_deallocate_all {
                ...
                template<typename U>
                static constexpr bool test(Check<U, &U::deallocate_all> *) { return true; }
