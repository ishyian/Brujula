...
typedef struct Baselib_CappedSemaphore {
    ...
    char _cachelineSpacer[PLATFORM_CACHE_LINE_SIZE - sizeof(int32_t) * 2 -
                          sizeof(Baselib_SystemSemaphore_Handle)];
    char _systemSemaphoreData[Baselib_SystemSemaphore_PlatformSize];
} Baselib_CappedSemaphore;

BASELIB_STATIC_ASSERT((offsetof(Baselib_CappedSemaphore, count) + PLATFORM_CACHE_LINE_SIZE -
                       sizeof(Baselib_SystemSemaphore_Handle)) ==
                      offsetof(Baselib_CappedSemaphore, _systemSemaphoreData),
                      "count and internalData must not share cacheline");

BASELIB_INLINE_API Baselib_CappedSemaphore
Baselib_CappedSemaphore_Create(uint16_t
cap)
{
Baselib_CappedSemaphore semaphore = {{0}, 0, cap, {0}, {0}};
semaphore.
handle = Baselib_SystemSemaphore_CreateInplace(&semaphore._systemSemaphoreData);
return
semaphore;
}
