...
namespace baselib {
    ...
    sizeof(intptr_t) * 2)mpmc_node_stack {
    public:
    ...
    void push_back(T * node) {
        ...
        if (PLATFORM_LLSC_NATIVE_SUPPORT) {
            ...
            } while (!m_Top.compare_exchange_strong(top, newtop, memory_order_release,
    ...
    T *try_pop_back() {
        ...
        if (PLATFORM_LLSC_NATIVE_SUPPORT) {
            ...
            do {
                ...
                newtop.idx = top.idx + 1;
            } while (!m_Top.compare_exchange_strong(top, newtop, memory_order_acquire,
                                                    memory_order_relaxed));
        }
        return node;
    }
    ...
    "Node class/struct used with baselib::mpmc_node_stack must derive from baselib::mpmc_node.");
};
}
}
