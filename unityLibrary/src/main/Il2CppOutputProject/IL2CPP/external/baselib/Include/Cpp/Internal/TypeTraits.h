...
// this can't be put inside compiler env due to __GLIBCXX__ not being set at that point
    ...
    struct is_trivial_of_size : std::integral_constant<bool,
            is_trivially_copyable<T>::value && (sizeof(T) == S)> {
    };
    template<typename T, size_t S>
    struct is_integral_of_size : std::integral_constant<bool,
            std::is_integral<T>::value && (sizeof(T) == S)> {
    };

    template<typename T, typename T2>
    struct is_of_same_signedness : std::integral_constant<bool,
