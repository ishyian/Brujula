...
namespace baselib {
    ...
    sizeof(intptr_t) * 2)mpsc_node_stack {
    public:
    ...
    T *try_pop_back() {
        ...
        if (PLATFORM_LLSC_NATIVE_SUPPORT) {
            ...
            do {
                ...
                newtop.idx = top.idx + 1;
            } while (!m_Top.compare_exchange_strong(top, newtop, memory_order_relaxed,
                                                    memory_order_relaxed));
        }
    ...
    static_assert(std::is_base_of<baselib::mpsc_node, T>::value,
    "Node class/struct used with baselib::mpsc_node_stack must derive from baselib::mpsc_node.");
};
}
}
