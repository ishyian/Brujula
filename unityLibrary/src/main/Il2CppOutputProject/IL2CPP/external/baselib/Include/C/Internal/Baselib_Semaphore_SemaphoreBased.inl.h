...
typedef struct Baselib_Semaphore {
    ...
    char _cachelineSpacer0[
            PLATFORM_CACHE_LINE_SIZE - sizeof(int32_t) - sizeof(Baselib_SystemSemaphore_Handle)];
    char _systemSemaphoreData[Baselib_SystemSemaphore_PlatformSize];
} Baselib_Semaphore;

BASELIB_STATIC_ASSERT((offsetof(Baselib_Semaphore, count) + PLATFORM_CACHE_LINE_SIZE -
                       sizeof(Baselib_SystemSemaphore_Handle)) ==
                      offsetof(Baselib_Semaphore, _systemSemaphoreData),
