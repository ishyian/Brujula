...
BASELIB_INLINE_API Baselib_HighCapacitySemaphore

Baselib_HighCapacitySemaphore_Create(void) {
    Baselib_HighCapacitySemaphore semaphore = {0, {0}, 0, {0}};
    return semaphore;
}

BASELIB_INLINE_API bool
Detail_Baselib_HighCapacitySemaphore_ConsumeWakeup(Baselib_HighCapacitySemaphore * semaphore) {
    int32_t previousCount = Baselib_atomic_load_32_relaxed(&semaphore->wakeups);
    while (previousCount > 0) {
        if (Baselib_atomic_compare_exchange_weak_32_relaxed_relaxed(&semaphore->wakeups,
                                                                    &previousCount,
                                                                    previousCount - 1))
            return true;
    }
...
BASELIB_INLINE_API bool
Baselib_HighCapacitySemaphore_TryAcquire(Baselib_HighCapacitySemaphore * semaphore) {
    int64_t previousCount = Baselib_atomic_load_64_relaxed(&semaphore->count);
    while (previousCount > 0) {
        if (Baselib_atomic_compare_exchange_weak_64_acquire_relaxed(&semaphore->count,
                                                                    &previousCount,
                                                                    previousCount - 1))
            return true;
    }
...
Baselib_HighCapacitySemaphore_Acquire(Baselib_HighCapacitySemaphore * semaphore) {
...
BASELIB_INLINE_API bool Baselib_HighCapacitySemaphore_TryTimedAcquire(
...
const int64_t previousCount = Baselib_atomic_fetch_add_64_acquire(&semaphore->count, -1);
if (
OPTIMIZER_LIKELY(previousCount
> 0))
return true;
...
do
{
Baselib_SystemFutex_Wait(&semaphore
->wakeups, 0, timeLeft);
if (
Detail_Baselib_HighCapacitySemaphore_ConsumeWakeup(semaphore)
)
return true;
...
{
if (
Baselib_atomic_compare_exchange_weak_64_relaxed_relaxed(&semaphore
->count, &count, count + 1))
return false;
}
...
BASELIB_INLINE_API void Baselib_HighCapacitySemaphore_Release(
        Baselib_HighCapacitySemaphore * semaphore,
...
BaselibAssert(previousCount
<= (previousCount + count), "Semaphore count overflow (current: %d, added: %d).", (int32_t)previousCount, (int32_t)count);

if (
OPTIMIZER_UNLIKELY(previousCount<
0))
{
const int64_t waitingThreads = -previousCount;
const int64_t threadsToWakeup = count < waitingThreads ? count : waitingThreads;
BaselibAssert(threadsToWakeup
<= INT32_MAX);
Baselib_atomic_fetch_add_32_relaxed(&semaphore
->wakeups, (int32_t)threadsToWakeup);
Baselib_SystemFutex_Notify(&semaphore
->wakeups, (int32_t)threadsToWakeup, Baselib_WakeupFallbackStrategy_OneByOne);
return;
}
...
while (
OPTIMIZER_UNLIKELY(previousCount
> Baselib_HighCapacitySemaphore_MaxGuaranteedCount * 2))
{
const int64_t maxCount = Baselib_HighCapacitySemaphore_MaxGuaranteedCount;
if (
Baselib_atomic_compare_exchange_weak_64_relaxed_relaxed(&semaphore
->count, &previousCount, maxCount))
return;
}
}

BASELIB_INLINE_API uint64_t
Baselib_HighCapacitySemaphore_ResetAndReleaseWaitingThreads(Baselib_HighCapacitySemaphore
* semaphore)
{
const int64_t count = Baselib_atomic_exchange_64_release(&semaphore->count, 0);
if (
OPTIMIZER_LIKELY(count
>= 0))
return 0;
...
->wakeups, (int32_t)threadsToWakeup);
Baselib_SystemFutex_Notify(&semaphore
->wakeups, (int32_t)threadsToWakeup, Baselib_WakeupFallbackStrategy_All);
return
threadsToWakeup;
}

BASELIB_INLINE_API void
Baselib_HighCapacitySemaphore_Free(Baselib_HighCapacitySemaphore * semaphore) {
    if (!semaphore)
        return;
    const int64_t count = Baselib_atomic_load_64_seq_cst(&semaphore->count);
    BaselibAssert(count >= 0,
                  "Destruction is not allowed when there are still threads waiting on the semaphore.");
}
