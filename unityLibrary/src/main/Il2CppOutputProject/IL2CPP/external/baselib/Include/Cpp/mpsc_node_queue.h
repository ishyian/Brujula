...
namespace baselib {
    ...
    sizeof(intptr_t) * 2)mpsc_node_queue {
    public:
    ...
    T *try_pop_front() {
        ...
        if (!next) {
            ...
            expected = node;
            if (!m_Back.compare_exchange_strong(expected, 0, memory_order_acquire,
    ...
    "Node class/struct used with baselib::mpsc_node_queue must derive from baselib::mpsc_node.");
};
}
}
