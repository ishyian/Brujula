...
namespace baselib {
    ...
    public:
        ...
        CappedSemaphore(const uint16_t cap) : m_CappedSemaphoreData(
                Baselib_CappedSemaphore_Create(cap)) {
        }
        ...
        inline bool TryTimedAcquire(const timeout_ms timeoutInMilliseconds) {
            return Baselib_CappedSemaphore_TryTimedAcquire(&m_CappedSemaphoreData,
