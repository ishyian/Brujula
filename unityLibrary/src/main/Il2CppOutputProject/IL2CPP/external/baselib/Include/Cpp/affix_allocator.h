...
namespace baselib {
    ...
    public:
        ...
        void *prefix(void *ptr) const {
            return prefix_size == 0 ? nullptr : OffsetPtr(ptr,
                                                          -static_cast<ptrdiff_t>(prefix_size));
        }
    ...
    private:
        ...
        AlignSize(size_t
        size) { return (size + Allocator::alignment - 1) & ~(Allocator::alignment - 1); }

        static FORCE_INLINE constexpr

        void *OffsetPtrChecked(const void *ptr, size_t offset) {
            return ptr == nullptr ? nullptr : OffsetPtr(ptr, offset);
        }

        static FORCE_INLINE constexpr

        void *OffsetPtr(const void *ptr, size_t offset) {
        ...
        FORCE_INLINE void *ReallocateImpl(void *ptr, size_t old_size, size_t new_size) {
            return OffsetPtrChecked(Allocator::reallocate(OffsetPtr(ptr, -m_PrefixAlignedSize),
                                                          old_size + m_PrefixAlignedSize,
                                                          new_size + m_PrefixAlignedSize),
                                    m_PrefixAlignedSize);
        }
        ...
        FORCE_INLINE void *ReallocateImpl(void *ptr, size_t old_size, size_t new_size) {
            ...
            ptr = Allocator::reallocate(OffsetPtr(ptr, -m_PrefixAlignedSize),
                                        old_size + m_AffixSize, new_size + m_AffixSize);
            if (ptr) {
        ...
        m_PrefixAlignedSize = AlignSize(prefix_size);
        static constexpr ptrdiff_t
        m_SuffixSize = suffix_size;
        static constexpr ptrdiff_t
        m_AffixSize = m_PrefixAlignedSize + m_SuffixSize;
    };
}
}
