...
namespace baselib {
    ...
    namespace Algorithm {
        ...
        inline int HighestBit(uint64_t value) {
            ...
            return _BitScanReverse(&upper, (uint32_t)(value >> 32)) ? (int) (32 + upper)
                                                                    : lower_int;
#endif
        }
        ...
        inline int BitsInMask(uint64_t value) {
            ...
            value = value - ((value >> 1) & (uint64_t)
            ~(uint64_t) 0 / 3);
            value = (value & (uint64_t)
            ~(uint64_t) 0 / 15 * 3) +((value >> 2) & (uint64_t)
            ~(uint64_t) 0 / 15 * 3);
            value = (value + (value >> 4)) & (uint64_t)
            ~(uint64_t) 0 / 255 * 15;
            return (uint64_t)(value * ((uint64_t)
            ~(uint64_t) 0 / 255)) >> (sizeof(uint64_t) - 1) * 8;
        }
