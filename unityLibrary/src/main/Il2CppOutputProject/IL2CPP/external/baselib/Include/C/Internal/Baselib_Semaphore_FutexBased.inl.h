...
BASELIB_STATIC_ASSERT(sizeof(Baselib_Semaphore) == PLATFORM_CACHE_LINE_SIZE * 2,
                      "Baselib_Semaphore (Futex) size should match 2*cacheline size (128bytes)");

BASELIB_STATIC_ASSERT(offsetof(Baselib_Semaphore, wakeups) ==
                      (offsetof(Baselib_Semaphore, count) - PLATFORM_CACHE_LINE_SIZE),
                      "Baselib_Semaphore (Futex) wakeups and count shouldnt share cacheline");

BASELIB_INLINE_API Baselib_Semaphore

Baselib_Semaphore_Create(void) {
...
BASELIB_INLINE_API bool Baselib_Semaphore_TryTimedAcquire(Baselib_Semaphore * semaphore,
...
->wakeups, 0, timeLeft);
if (
Detail_Baselib_Semaphore_ConsumeWakeup(semaphore)
)
return true;
...
BASELIB_INLINE_API void Baselib_Semaphore_Release(Baselib_Semaphore * semaphore,
...
<= (previousCount + count), "Semaphore count overflow (current: %d, added: %d).", previousCount, count);
...
while (
OPTIMIZER_UNLIKELY(previousCount
> Baselib_Semaphore_MaxGuaranteedCount * 2))
{
...
Baselib_Semaphore_ResetAndReleaseWaitingThreads(Baselib_Semaphore
* semaphore)
{
