...
namespace baselib {
    ...
    enum memory_order_relaxed_t {
        memory_order_relaxed = 0
    }; // Equal to std::memory_order_relaxed
    enum memory_order_acquire_t {
        memory_order_acquire = 2
    }; // Equal to std::memory_order_acquire
    enum memory_order_release_t {
        memory_order_release = 3
    }; // Equal to std::memory_order_release
    enum memory_order_acq_rel_t {
        memory_order_acq_rel = 4
    }; // Equal to std::memory_order_acq_rel
    enum memory_order_seq_cst_t {
        memory_order_seq_cst = 5
    }; // Equal to std::memory_order_seq_cst
...
#define TEST_ATOMICS_PREREQUISITES(_TYPE) \
        ...
        template<typename T>
        static inline T fail();

        template<typename T, typename MemoryOrder, typename ... AllowedMemoryOrders>
        static inline T fail_prerequisites() {
            TEST_ATOMICS_PREREQUISITES(T);
            static_assert(is_any<MemoryOrder, AllowedMemoryOrders...>::value,
                          "the specified memory ordering is invalid for this atomic operation");
            return fail<T>();
        }

        template<typename T, typename MemoryOrderSuccess, typename MemoryOrderFailure>
        static inline T fail_prerequisites_cmpxchg() {
            TEST_ATOMICS_PREREQUISITES(T);
            static_assert(
                    ...
                    (std::is_same<MemoryOrderSuccess, baselib::memory_order_seq_cst_t>::value &&
                     std::is_same<MemoryOrderFailure, baselib::memory_order_seq_cst_t>::value),
        ...
        static inline T fail_prerequisites_alu() {
            static_assert(std::is_integral<T>::value,
                          "operands of arithmetic atomic operations need to be integral");
...
#undef detail_NOT_SUPPORTED
    ...
    atomic_fetch_sub_explicit(T
    & obj,
    typename std::common_type<T>::type value, MemoryOrder
    order) {
    return
    atomic_fetch_add_explicit(obj,
    0 - value, order);
}
...
atomic_load_explicit(const T &obj, MemoryOrder order) {
...
atomic_store_explicit(T &obj, typename std::common_type<T>::type value, MemoryOrder order) {
...
detail::fail_prerequisites_alu<T, MemoryOrder>();

}

template<typename T, typename MemoryOrder>
static FORCE_INLINE T
atomic_fetch_and_explicit(T
& obj,
typename std::common_type<T>::type value, MemoryOrder
order)
{
...
detail::fail_prerequisites<T, MemoryOrder>();
...
static FORCE_INLINE bool atomic_compare_exchange_weak(T &obj,
                                                      typename std::common_type<T>::type &expected,
                                                      typename std::common_type<T>::type desired) {
    return atomic_compare_exchange_weak_explicit(obj, expected, desired, memory_order_seq_cst,
...
static FORCE_INLINE bool atomic_compare_exchange_strong(T &obj,
                                                        typename std::common_type<T>::type &expected,
                                                        typename std::common_type<T>::type desired) {
    return atomic_compare_exchange_strong_explicit(obj, expected, desired, memory_order_seq_cst,
                                                   memory_order_seq_cst);
}
...
struct atomic_common {
    ...
    exchange(T
    value,
    TMemoryOrder order = memory_order_seq_cst
    )
    {
    ...
    FORCE_INLINE bool
    compare_exchange_weak(T &expected, T desired, TMemoryOrderSuccess order_success,
                          TMemoryOrderFailure order_failure) {
        return atomic_compare_exchange_weak_explicit(obj, expected, desired, order_success,
                                                     order_failure);
    }
    ...
    template<typename TMemoryOrderSuccess, typename TMemoryOrderFailure>
    FORCE_INLINE bool
    compare_exchange_strong(T &expected, T desired, TMemoryOrderSuccess order_success,
                            TMemoryOrderFailure order_failure) {
        return atomic_compare_exchange_strong_explicit(obj, expected, desired, order_success,
                                                       order_failure);
    }
...
struct atomic_base<T, true> : atomic_common<T> {
    ...
    operator++(int) {
        return atomic_fetch_add_explicit(atomic_common<T>::obj, T(1), memory_order_seq_cst);
    }

    FORCE_INLINE T

    operator--(int) {
        return atomic_fetch_sub_explicit(atomic_common<T>::obj, T(1), memory_order_seq_cst);
    }

    FORCE_INLINE T

    operator++() {
        return atomic_fetch_add_explicit(atomic_common<T>::obj, T(1), memory_order_seq_cst) + T(1);
    }

    FORCE_INLINE T

    operator--() {
        return atomic_fetch_sub_explicit(atomic_common<T>::obj, T(1), memory_order_seq_cst) - T(1);
    }

    FORCE_INLINE T

    operator+=(T value) {
        return atomic_fetch_add_explicit(atomic_common<T>::obj, value, memory_order_seq_cst) +
               value;
    }
    ...
    operator-=(T value) {
        return atomic_fetch_sub_explicit(atomic_common<T>::obj, value, memory_order_seq_cst) -
               value;
    }
    ...
    operator&=(T value) {
        return atomic_fetch_and_explicit(atomic_common<T>::obj, value, memory_order_seq_cst) &
               value;
    }
    ...
    operator|=(T value) {
        return atomic_fetch_or_explicit(atomic_common<T>::obj, value, memory_order_seq_cst) | value;
    }

    FORCE_INLINE T

    operator^=(T value) {
        return atomic_fetch_xor_explicit(atomic_common<T>::obj, value, memory_order_seq_cst) ^
               value;
    }
};
