...
namespace baselib {
    ...
    namespace Algorithm {
        ...
        template<typename T, typename std::enable_if<
                std::is_same<size_t, T>::value && sizeof(T) == 4, bool>::type = 0>
        ...
        template<typename T, typename std::enable_if<
                std::is_same<size_t, T>::value && sizeof(T) == 8, bool>::type = 0>
        ...
        template<typename T, typename std::enable_if<
                std::is_same<size_t, T>::value && sizeof(T) == 4, bool>::type = 0>
        inline int HighestBitNonZero(T value) {
            return HighestBitNonZero(static_cast<uint32_t>(value));
        }
        ...
        template<typename T, typename std::enable_if<
                std::is_same<size_t, T>::value && sizeof(T) == 8, bool>::type = 0>
        inline int HighestBitNonZero(T value) {
            return HighestBitNonZero(static_cast<uint64_t>(value));
        }
        ...
        template<typename T, typename std::enable_if<
                std::is_same<size_t, T>::value && sizeof(T) == 4, bool>::type = 0>
        inline int LowestBitNonZero(T value) {
            return LowestBitNonZero(static_cast<uint32_t>(value));
        }
        ...
        template<typename T, typename std::enable_if<
                std::is_same<size_t, T>::value && sizeof(T) == 8, bool>::type = 0>
        inline int LowestBitNonZero(T value) {
            return LowestBitNonZero(static_cast<uint64_t>(value));
        }
...
#undef HarleySealCSAStep
        ...
        constexpr inline bool AreIntegersMultiple(T a, T b) {
            static_assert(std::is_integral<T>::value,
                          "AreIntegersMultiple requires integral types.");
            return a != 0 && b != 0 &&
                   // if at least one integer is 0, consider false (avoid div by 0 of the following modulo)
                   ((a % b) == 0 || (b % a) == 0);
        }
        ...
        constexpr inline bool IsPowerOfTwo(T value) {
            static_assert(std::is_integral<T>::value,
                          "IsPowerOfTwo works only with an integral type.");
    ...
    CeilPowerOfTwo(T
    value) {
    return CeilPowerOfTwo(static_cast
    <uint32_t>(value)
    );
}
...
"RoundPowerOfTwo works only with an unsigned integral type.");
return (value - (
...
{
static_assert(std::is_integral<T>::value,
"CeilAligned works only with an integral type.");
return static_cast<T>((static_cast
...
constexpr inline bool DoesAdditionOverflow(T a, T b) {
    static_assert(std::is_unsigned<T>::value,
                  "Overflow checks apply only work on unsigned integral types.");
...
template<typename RT, typename VT, typename std::enable_if<
        ...
        || !std::is_integral<VT>::value, bool>::type = 0>
...
template<typename RT, typename VT, typename std::enable_if<
        ...
        std::is_integral<VT>::value && std::is_signed<VT>::value, bool>::type = 0>
inline RT Clamp(VT value, RT lo, RT hi) {
    if (value < 0)
        return lo;
...
template<typename RT, typename VT, typename std::enable_if<
        ...
        std::is_integral<VT>::value && std::is_unsigned<VT>::value, bool>::type = 0>
inline RT Clamp(VT value, RT lo, RT hi) {
    if (hi < 0)
        return hi;
    if (lo < 0)
...
//
template<typename RT, typename VT, typename std::enable_if<
        !(std::numeric_limits<RT>::has_infinity &&
          std::numeric_limits<VT>::has_infinity), bool>::type = 0>
...
template<typename RT, typename VT, typename std::enable_if<
        (std::numeric_limits<RT>::has_infinity &&
         std::numeric_limits<VT>::has_infinity), bool>::type = 0>
inline RT ClampToType(VT value) {
    if (value == std::numeric_limits<VT>::infinity() ||
        value == -std::numeric_limits<VT>::infinity())
        return static_cast<RT>(value);
    return Clamp(value, std::numeric_limits<RT>::lowest(), std::numeric_limits<RT>::max());
}
