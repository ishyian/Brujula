...
namespace baselib {
    ...
    public:
        ...
        mpmc_fixed_queue(uint32_t capacity, void *buffer = nullptr)
                : m_SlotAllocator(),
                  m_Slot(static_cast<Slot *>(buffer ? buffer : m_SlotAllocator.allocate(
                          buffer_size(capacity)))), m_UserAllocatedSlots(buffer ? nullptr : m_Slot),
                  m_NumberOfSlots(capacity ? capacity : 2), m_Capacity(capacity), m_ReadPos(0),
                  m_WritePos(0) {
            ...
            if (capacity == 0) {
                ...
                for (uint32_t pos = 0; pos < capacity; ++pos)
                    m_Slot[pos].checksum.store(WriteableChecksum(pos),
        ...
        ~mpmc_fixed_queue() {
            ...
            m_SlotAllocator.deallocate(m_UserAllocatedSlots,
                                       buffer_size(static_cast<uint32_t>(m_Capacity)));
            baselib::atomic_thread_fence(baselib::memory_order_seq_cst);
        }
        ...
        bool try_pop_front(value_type &value) {
            while (true) {
                ...
                while (checksum == ReadableChecksum(pos)) {
                    ...
                    if (m_ReadPos.compare_exchange_weak(pos, pos + 1, baselib::memory_order_relaxed,
                                                        baselib::memory_order_relaxed)) {
                        value = std::move(slot->value);
                        slot->value.~value_type();
                        slot->checksum.store(WriteableChecksumNextGen(pos),
                                             baselib::memory_order_release);
                        return true;
                    }
        ...
        bool try_emplace_back(Args &&... args) {
            while (true) {
                ...
                while (checksum == WriteableChecksum(pos)) {
                    ...
                    if (m_WritePos.compare_exchange_weak(pos, pos + 1,
                                                         baselib::memory_order_relaxed,
                                                         baselib::memory_order_relaxed)) {
    ...
    private:
        ...
        MinTypeAlignment =
        alignof(value_type) > sizeof(void *) ? alignof(value_type) : sizeof(void *);
        static constexpr uint32_t
        SlotAlignment =
        cacheline_aligned && PLATFORM_CACHE_LINE_SIZE > MinTypeAlignment ? PLATFORM_CACHE_LINE_SIZE
                                                                         : MinTypeAlignment;
        static constexpr uint32_t
        ReadableBit = (uint32_t)
        1 << 31;
        static constexpr uint32_t
        WritableMask = ~ReadableBit;
        static constexpr uint32_t
        WriteableChecksum(uint32_t
        pos)       { return pos & WritableMask; }
        static constexpr uint32_t
        ReadableChecksum(uint32_t
        pos)        { return pos | ReadableBit; }
        constexpr uint32_t
        WriteableChecksumNextGen(uint32_t
        pos) const { return (pos + m_NumberOfSlots) & WritableMask; }
        constexpr uint32_t
        ReadableChecksumPrevGen(uint32_t
        pos) const  { return (pos - m_NumberOfSlots) | ReadableBit; }

        constexpr uint32_t
        SlotIndex(uint32_t
        pos) const           { return pos % m_NumberOfSlots; }

        const baselib::heap_allocator<SlotAlignment> m_SlotAllocator;

        struct alignas(SlotAlignment)
        Slot
        {
            value_type value;
        ...
        baselib::atomic<uint32_t> m_ReadPos;
        alignas(PLATFORM_CACHE_LINE_SIZE)
        baselib::atomic<uint32_t> m_WritePos;
    };
}
}
