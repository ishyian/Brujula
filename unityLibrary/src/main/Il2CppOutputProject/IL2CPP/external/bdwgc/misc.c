...
GC_default_oom_fn(
        size_t
bytes_requested GC_ATTR_UNUSED
)
{
return(0);
}
...
# else
STATIC word
GC_stack_last_cleared = 0; /* GC_no when we last did this */
STATIC ptr_t
GC_min_sp = NULL;
...
GC_CALL GC_get_free_bytes(void) {
...
GC_CALL GC_get_unmapped_bytes(void) {
    return (size_t)
    GC_unmapped_bytes;
}
...
GC_CALL GC_get_bytes_since_gc(void) {
    return (size_t)
    GC_bytes_allocd;
}
...
GC_CALL GC_get_total_bytes(void) {
...
GC_CALL GC_get_size_map_at(int i) {
...
*pheap_size,
GC_word *pfree_bytes, GC_word
*punmapped_bytes,
GC_word *pbytes_since_gc, GC_word
*ptotal_bytes)
{
DCL_LOCK_STATE;
...
GC_INNER word
GC_reclaimed_bytes_before_gc = 0;
...
GC_CALL GC_get_prof_stats(struct GC_prof_stats_s *pstats,
                          size_t stats_sz) {
...
GC_INNER GC_warn_proc
GC_current_warn_proc = GC_default_warn_proc;

/* This is recommended for production code (release). */
GC_API void GC_CALLBACK

GC_ignore_warn_proc(char *msg, GC_word arg) {
    if (GC_print_stats) {
...
GC_API void GC_CALL
GC_set_abort_func(GC_abort_func
fn)
{
DCL_LOCK_STATE;
...
GC_API GC_abort_func

GC_CALL GC_get_abort_func(void) {
    GC_abort_func fn;
    DCL_LOCK_STATE;
    LOCK();
    fn = GC_on_abort;
...
GC_new_kind_inner(void **fl, GC_word descr,
    ...
    if (result < MAXOBJKINDS) {
        ...
        GC_obj_kinds[result].ok_init = (GC_bool)
        clear;
...
GC_new_kind(void **fl, GC_word descr, int adjust,
            int clear) {
    unsigned result;
    DCL_LOCK_STATE;
    LOCK();
...
GC_API unsigned GC_CALL
GC_new_proc_inner(GC_mark_proc
proc)
{
...
GC_API void *GC_CALL
GC_call_with_alloc_lock(GC_fn_type
fn,
void *client_data
)
{
...
GC_API void *GC_CALL
GC_call_with_stack_base(GC_stack_base_func
fn,
void *arg
)
{
...
GC_API void *GC_CALL
GC_call_with_gc_active(GC_fn_type
fn,
void *client_data
)
{
...
GC_stackbottom HOTTER_THAN(word)(
...
#   endif
stacksect.
prev = GC_traced_stack_sect;
GC_blocked_sp = NULL;
GC_traced_stack_sect = &stacksect;

client_data = fn(client_data);
GC_ASSERT(GC_blocked_sp
== NULL);
GC_ASSERT(GC_traced_stack_sect
== &stacksect);
...
GC_with_callee_saves_pushed(GC_do_blocking_inner, (ptr_t)(
&my_data));
return my_data.
client_data; /* result */
}
...
static void block_add_size(struct hblk *h, word pbytes) {
    hdr *hhdr = HDR(h);
    *(word * )
    pbytes += (WORDS_TO_BYTES(hhdr->hb_sz) + (HBLKSIZE - 1))
              & ~(word)(HBLKSIZE - 1);
}

GC_API size_t

GC_CALL GC_get_memory_use(void) {
    word bytes = 0;
    DCL_LOCK_STATE;
...
GC_API void GC_CALL
GC_set_on_heap_resize(GC_on_heap_resize_proc
fn)
{
/* fn may be 0 (means no event notifier). */
DCL_LOCK_STATE;
...
GC_API GC_on_heap_resize_proc

GC_CALL GC_get_on_heap_resize(void) {
...
GC_CALL GC_get_finalizer_notifier(void) {
...
GC_CALL GC_get_non_gc_bytes(void) {
    return GC_non_gc_bytes;
}

GC_API void GC_CALL
GC_set_free_space_divisor(GC_word
value)
{
GC_ASSERT(value
> 0);
...
GC_CALL GC_get_free_space_divisor(void) {
    return GC_free_space_divisor;
}

GC_API void GC_CALL
GC_set_max_retries(GC_word
value)
{
GC_ASSERT(value
!= ~(word)0);
...
GC_CALL GC_get_max_retries(void) {
...
GC_set_time_limit_ns(unsigned long long value) {
    GC_ASSERT(value != (unsigned long long) -1L);
    GC_time_limit = value;
}

GC_API unsigned long long GC_CALL

GC_get_time_limit_ns(void) {
...
GC_set_force_unmap_on_gcollect(int value) {
    GC_force_unmap_on_gcollect = (GC_bool)
    value;
}
