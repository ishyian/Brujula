...
# else
    ...
    last_bit = FINAL_MARK_BIT((size_t)
    hhdr->hb_sz);
# endif
...
GC_INNER void GC_clear_marks(void) {
    GC_apply_to_all_blocks(clear_marks_for_block, (word)
    0);
    GC_objects_are_marked = FALSE;
...
mse *GC_mark_from(mse *mark_stack_top, mse *mark_stack,
...
# else
    while (((((word)mark_stack_top - (word)
    mark_stack) | (word)
    credit)
    &SIGNB) == 0)
# endif
...
#         endif /* ENABLE_TRACE */
                    ...
                    while (descr != 0) {
                        if ((descr & SIGNB) != 0) {
                            ...
                            if (current >= (word)least_ha && current < (word)
                            greatest_ha) {
                                PREFETCH((ptr_t)
                                current);
#               ifdef ENABLE_TRACE
...
#         endif /* ENABLE_TRACE */
                    ...
                    mark_stack_top = (*PROC(descr))((word * )
...
#     endif
            limit = current_p + (word)
            descr;
        }
        /* The simple case in which we're scanning a range. */
        GC_ASSERT(!((word)
        current_p & (ALIGNMENT - 1)));
...
#     ifndef SMALL_CONFIG
            ...
            for (;;) {
                ...
                limit -= ALIGNMENT;
                if (deferred >= (word)least_ha && deferred < (word)
                greatest_ha) {
                    PREFETCH((ptr_t)
                    deferred);
                    break;
                }
                if ((word)current_p > (word)
                limit) goto next_object;
...
GC_INNER void GC_scratch_recycle_inner(void *ptr, size_t bytes) {
    if (ptr != NULL) {
        ...
        GC_COND_LOG_PRINTF("Recycle %lu/%lu scratch-allocated bytes at %p\n",
                           ...
                           ptr);
        if (recycled_bytes > 0)
            GC_add_to_heap((struct hblk *) ((word)
        ptr + displ), recycled_bytes);
    }
}
...
static void alloc_mark_stack(size_t n) {
    mse * new_stack = (mse * )
    GC_scratch_alloc(n * sizeof(struct GC_ms_entry));
#   ifdef GWW_VDB
...
#   endif
    ...
    if (GC_mark_stack != NULL) {
        if (new_stack != 0) {
            ...
            WARN("Failed to grow mark stack to %"
            WARN_PRIdPTR
            " frames\n", n);
        }
    } else {
...
GC_push_all(void *bottom, void *top) {
    ...
    bottom = (void *) (((word)
    bottom + ALIGNMENT - 1) &~(ALIGNMENT - 1));
    top = (void *) ((word)
    top & ~(ALIGNMENT - 1));
    if ((word)bottom >= (word)
    top) return;
    ...
    GC_mark_stack_top++;
    if ((word)GC_mark_stack_top >= (word)
    GC_mark_stack_limit) {
...
#   if GC_DS_TAGS > ALIGNMENT - 1
    ...
    length &= ~GC_DS_TAGS;
#   endif
    GC_mark_stack_top->mse_start = (ptr_t)
    bottom;
...
GC_push_conditional(void *bottom, void *top, int all) {
    if (!all) {
        GC_push_selected((ptr_t)
        bottom, (ptr_t)
        top, GC_page_was_dirty);
    } else {
#     ifdef PROC_VDB
        if (GC_incremental) {
...
GC_mark_and_push(void *obj,
    ...
    if ((EXPECT(IS_FORWARDING_ADDR_OR_NIL(hhdr), FALSE)
         && (!GC_all_interior_pointers
             || NULL == (hhdr = GC_find_header((ptr_t)
        GC_base(obj)))))
    || EXPECT(HBLK_IS_FREE(hhdr), FALSE)) {
        GC_ADD_TO_BLACK_LIST_NORMAL(obj, (ptr_t)
        src);
...
GC_block_was_dirty(struct hblk *h, hdr *hhdr) {
    word sz = hhdr->hb_sz;
