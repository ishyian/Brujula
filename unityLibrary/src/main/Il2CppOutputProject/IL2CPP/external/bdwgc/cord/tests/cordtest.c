...
#   define FNAME2 "cordtst2.tmp"
    ...
    if (CORD_cmp(CORD_substr(w, 50 * 36 + 2, 36), y) != 0) ABORT("file substr wrong");
    f1b = fopen(FNAME1, "rb");
...
#   endif
    ...
    if (CORD_cmp(CORD_substr(w, 1000 * 36, 36), y) != 0) ABORT("file substr wrong");
    if (strcmp(CORD_to_char_star(CORD_substr(w, 1000 * 36, 36)), y) != 0) ABORT(
    ...
    if (CORD_str(x, 0, "9abcdefghijx") != CORD_NOT_FOUND) ABORT("CORD_str failed 3");
...
void test_printf(void) {
    ...
    if (CORD_sprintf(&result, "%7.2f%ln", 3.14159F, &l) != 7) ABORT("CORD_sprintf failed 1");
    if (CORD_cmp(result, "   3.14") != 0) ABORT("CORD_sprintf goofed 1");
    if (l != 7) ABORT("CORD_sprintf goofed 2");
    if (CORD_sprintf(&result, "%-7.2s%hn%c%s", "abcd", &s, 'x', "yz") != 10) ABORT(
    ...
    x = CORD_cat(x, x);
    x = CORD_cat(x, x);
    if (CORD_sprintf(&result, "->%-120.78r!\n", x) != 124) ABORT("CORD_sprintf failed 3");
#   ifdef GC_SNPRINTF
