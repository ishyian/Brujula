...
#     ifndef GC_NO_FINALIZATION
        GC_REGISTER_FINALIZER((void *) result, finalizer, (void *) (GC_word)
        n,
...
void typed_test(void) {
    ...
    for (i = 0; i < 4000; i++) {
        newP = (GC_word * )
        GC_malloc_explicitly_typed(4 * sizeof(GC_word), d1);
        ...
        AO_fetch_and_add1(&collectable_count);
        newP = (GC_word * )
        GC_malloc_explicitly_typed(4 * sizeof(GC_word), d2);
        CHECK_OUT_OF_MEMORY(newP);
        newP[0] = 17;
        newP[1] = (GC_word)
        old;
        ...
        AO_fetch_and_add1(&collectable_count);
        newP = (GC_word * )
        GC_malloc_explicitly_typed(33 * sizeof(GC_word), d3);
        CHECK_OUT_OF_MEMORY(newP);
        newP[0] = 17;
        newP[1] = (GC_word)
        old;
...
fail_proc1(void *x GC_ATTR_UNUSED) {
    fail_count++;
}
...
reachable_objs_counter(void *obj, size_t size,
                       void *pcounter) {
    if (0 == size) {
...
reachable_objs_count_enumerator(void *pcounter) {
