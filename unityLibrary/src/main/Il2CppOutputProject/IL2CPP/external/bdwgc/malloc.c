...
#   endif
while (0 == h &&
GC_collect_or_expand(n_blocks, flags
!= 0, retry)) {
h = GC_allochblk(lb, k, flags);
retry = TRUE;
}
...
/* EXTRA_BYTES were already added to lb.                                */
STATIC ptr_t
GC_alloc_large_and_clear(size_t
lb,
int k,
unsigned flags
)
...
GC_INNER void *GC_generic_malloc_inner(size_t lb, int k) {
    ...
    if (SMALL_OBJ(lb)) {
        ...
        *opp = obj_link(op);
        ...
        op = (ptr_t)
        GC_alloc_large_and_clear(ADD_SLOP(lb), k, 0);
        if (op != NULL)
            GC_bytes_allocd += lb;
    }
...
{
return
GC_generic_malloc_uncollectable(lb, UNCOLLECTABLE
);
}
#endif
