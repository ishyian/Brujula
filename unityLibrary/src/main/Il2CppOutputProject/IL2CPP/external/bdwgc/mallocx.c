...
void *GC_CALL
GC_generic_or_special_malloc(size_t
lb,
int knd
)
{
...
#       endif
return
GC_generic_malloc_uncollectable(lb, knd
);
default:
return
GC_generic_malloc(lb, knd
);
}
}
...
GC_API GC_ATTR_MALLOC
void *GC_CALL
GC_generic_malloc_ignore_off_page(size_t
lb,
int k
)
{
...
{
return
GC_generic_malloc_ignore_off_page(lb, NORMAL
);
}
...
{
return
GC_generic_malloc_ignore_off_page(lb, PTRFREE
);
}
...
GC_CALL GC_get_expl_freed_bytes_since_gc(void) {
    return (size_t)
    GC_bytes_freed;
}
...
-> hb_sz == lb);
hhdr ->
hb_last_reclaimed = (unsigned short) GC_gc_no;
#           ifdef PARALLEL_MARK
if (GC_parallel) {
...
GC_bytes_allocd += HBLKSIZE - HBLKSIZE %
lb;
#         ifdef PARALLEL_MARK
if (GC_parallel) {
  GC_acquire_mark_lock();
...
GC_generic_malloc_many(lb, NORMAL,
&result);
return
result;
}
...
GC_STATIC_ASSERT(VALID_OFFSET_SZ
<= HBLKSIZE);
...
GC_change_stubborn(const void *p GC_ATTR_UNUSED) {
    /* Empty. */
}
