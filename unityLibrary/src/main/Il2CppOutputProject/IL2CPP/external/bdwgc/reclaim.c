...
#endif
STATIC ptr_t
GC_leaked[MAX_LEAKED] = {
NULL };
STATIC unsigned GC_n_leaked = 0;

GC_INNER GC_bool
GC_have_errors = FALSE;
...
STATIC GC_bool
GC_block_nearly_full(hdr
*hhdr)
{
return (hhdr -> hb_n_marks > 7 *
HBLK_OBJS(hhdr
-> hb_sz)/8);
}
...
GC_reclaim_clear(struct hblk *hbp, hdr *hhdr, word sz,
                 ptr_t list, signed_word *count) {
    word bit_no = 0;
    word *p, *q, *plim;
    signed_word n_bytes_found = 0;

    GC_ASSERT(hhdr == GC_find_header((ptr_t)
    hbp));
...
GC_reclaim_uninit(struct hblk *hbp, hdr *hhdr, word sz,
                  ptr_t list, signed_word *count) {
    word bit_no = 0;
    word *p, *plim;
    signed_word n_bytes_found = 0;
...
GC_reclaim_generic(struct hblk *hbp, hdr *hhdr, size_t sz,
                   GC_bool init, ptr_t list,
                   signed_word *count) {
    ptr_t result;

    GC_ASSERT(GC_find_header((ptr_t)
    hbp) == hhdr);
...
GC_print_free_list(int kind, size_t sz_in_granules) {
    void *flh_next;
    int n;
...
GC_apply_to_all_blocks(GC_do_enumerate_reachable_objects, (word)
&ed);
}
