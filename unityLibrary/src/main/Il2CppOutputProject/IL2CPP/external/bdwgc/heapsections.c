...
static struct hblk *GetNextFreeBlock(ptr_t ptr) {
    ...
    for (i = 0; i < N_HBLK_FLS + 1; i++) {
        ...
        for (freeBlock = GC_hblkfreelist[i];
             freeBlock != NULL; freeBlock = HDR(freeBlock)->hb_next) {
...
static void
CallHeapSectionCallback(void *user_data, ptr_t start, ptr_t end, GC_heap_section_proc callback) {
    hdr *hhdr = HDR(start);
    ...
    for (i = 0; i < GC_n_heap_sects; i++) {
        ...
        while (sectionStart < sectionEnd) {
            ...
            if (nextFreeBlock == NULL || (ptr_t) nextFreeBlock > sectionEnd) {
                ...
                if (sectionLength > 0)
                    CallHeapSectionCallback(user_data, sectionStart, sectionStart + sectionLength,
                                            callback);
