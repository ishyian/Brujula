...
GC_is_static_root(void *p) {
    ...
    if (last_root_set < n_root_sets
        && (word)
        p >= (word)
    GC_static_roots[last_root_set].r_start
    && (word)
    p < (word)
...
void GC_add_roots_inner(ptr_t b, ptr_t e, GC_bool tmp) {
    ...
    e);
    b = (ptr_t)(((word)
    b + (sizeof(word) - 1)) &~(word)(sizeof(word) - 1));
    /* round b up to word boundary */
    e = (ptr_t)((word)
    e & ~(word)(sizeof(word) - 1));
    /* round e down to word boundary */
    if ((word)b >= (word)
    e) return; /* nothing to do */
...
STATIC void GC_remove_roots_inner(ptr_t b, ptr_t e) {
    ...
    for (i = 0; i < n_root_sets;) {
        if ((word)GC_static_roots[i].r_start >= (word)
        b
        && (word)
        GC_static_roots[i].r_end <= (word)
        e) {
            GC_remove_root_at_pos(i);
            rebuild = TRUE;
        } else {
...
STATIC size_t
GC_excl_table_entries = 0;/* Number of entries in use.      */
...
STATIC struct exclusion *GC_next_exclusion(ptr_t start_addr) {
    ...
    while (high > low) {
        ...
        /* low <= mid < high    */
        if ((word) GC_excl_table[mid].e_end <= (word)
        start_addr) {
            low = mid + 1;
        } else {
            high = mid;
    ...
    if ((word) GC_excl_table[low].e_end <= (word)
    start_addr) return 0;
...
GC_INNER void GC_exclude_static_roots_inner(void *start, void *finish) {
    ...
    GC_excl_table[next_index].e_start = (ptr_t)
    start;
    GC_excl_table[next_index].e_end = (ptr_t)
    finish;
...
GC_exclude_static_roots(void *b, void *e) {
    ...
    b = (void *) ((word)
    b & ~(word)(sizeof(word) - 1));
    e = (void *) (((word)
    e + (sizeof(word) - 1)) &~(word)(sizeof(word) - 1));
    if (NULL == e)
...
STATIC void GC_push_conditional_with_exclusions(ptr_t bottom, ptr_t top,
    ...
    while ((word)bottom < (word)
    top) {
        struct exclusion *next = GC_next_exclusion(bottom);
        ptr_t excl_start;

        if (0 == next
            || (word)(excl_start = next->e_start) >= (word)
            top) {
            GC_PUSH_CONDITIONAL(bottom, top, all);
            break;
        }
        if ((word)excl_start > (word)
        bottom)
...
#       ifdef STACK_GROWS_UP
        GC_push_all_stack_partially_eager((ptr_t)traced_stack_sect, lo,
                                          cold_gc_frame);
#       else /* STACK_GROWS_DOWN */
        GC_push_all_stack_partially_eager(lo, (ptr_t)
        traced_stack_sect,
                cold_gc_frame);
#       endif
        lo = traced_stack_sect->saved_stack_ptr;
