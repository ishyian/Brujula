...
if (
IS_FORWARDING_ADDR_OR_NIL(hhdr)
) {
...
hce ->
block_addr = (word)(p) >> LOG_HBLKSIZE;
hce ->
hce_hdr = hhdr;
return
hhdr;
...
" (%" WARN_PRIdPTR " bytes)...\n", (word)bytes);
scratch_free_ptr -=
bytes; /* Undo free area pointer update */
bytes_to_get = ROUNDUP_PAGESIZE_IF_MMAP(bytes);
result = (ptr_t)
GET_MEM(bytes_to_get);
GC_add_to_our_memory(result, bytes_to_get
...
GC_INNER void GC_init_headers(void) {
    ...
    if (GC_all_nils == NULL) {
        GC_all_nils = (bottom_index * )
        GC_scratch_alloc(sizeof(bottom_index));
        if (GC_all_nils == NULL) {
