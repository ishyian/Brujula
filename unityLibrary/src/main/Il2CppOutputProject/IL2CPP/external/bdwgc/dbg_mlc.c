...
GC_describe_type_fns[MAXOBJKINDS] = {
0};

GC_API void GC_CALL

GC_register_describe_type_fn(int kind,
...
# endif
    ...
    GC_register_displacement_inner((word)
    sizeof(oh));
}
...
GC_API GC_ATTR_MALLOC
void *GC_CALL
GC_debug_malloc(size_t
lb,
GC_EXTRA_PARAMS)
{
void *result;
...
GC_API void *GC_CALL
GC_debug_malloc_stubborn(size_t
lb, GC_EXTRA_PARAMS)
{
return
GC_debug_malloc(lb, OPT_RA
s, i);
}
...
GC_debug_strdup(const char *str,
                GC_EXTRA_PARAMS) {
    char *copy;
    size_t lb;
    if (str == NULL) {
    ...
    lb = strlen(str) + 1;
    copy = (char *) GC_debug_malloc_atomic(lb, OPT_RA
    s, i);
    if (copy == NULL) {
#   ifndef MSWINCE
        errno = ENOMEM;
...
GC_debug_strndup(const char *str,
                 size_t size, GC_EXTRA_PARAMS) {
    char *copy;
    size_t len = strlen(str); /* str is expected to be non-NULL  */
    if (len > size)
        len = size;
    copy = (char *) GC_debug_malloc_atomic(len + 1, OPT_RA
    s, i);
    if (copy == NULL) {
#   ifndef MSWINCE
        errno = ENOMEM;
...
#     ifndef SHORT_DBG_HDRS
        ptr_t
        clobbered = GC_check_annotated_obj((oh * )
        base);
...
#     endif /* SHORT_DBG_HDRS */
    ...
    if (GC_find_leak
        ...
        && ((ptr_t)
        p - (ptr_t)
    base != sizeof(oh) || !GC_findleak_delay_free)
#       endif
    ) {
        GC_free(base);
    } else {
...
#         endif
            ...
            for (i = 0; i < obj_sz; ++i)
                ((word * )
            p)[i] = GC_FREED_MEM_MARKER;
            GC_ASSERT((word * )
            p + i == (word * )(base + hhdr->hb_sz));
        }
    } /* !GC_find_leak */
}
...
GC_debug_realloc(void *p, size_t lb, GC_EXTRA_PARAMS) {
    ...
    if (p == 0) {
        return GC_debug_malloc(lb, OPT_RA
        s, i);
    }
    if (0 == lb) /* and p != NULL */ {
        GC_debug_free(p);
...
#   endif
    ...
    switch (hhdr->hb_obj_kind) {
        case NORMAL:
            result = GC_debug_malloc(lb, OPT_RA
            s, i);
            break;
        case PTRFREE:
            result = GC_debug_malloc_atomic(lb, OPT_RA
            s, i);
            break;
        case UNCOLLECTABLE:
            result = GC_debug_malloc_uncollectable(lb, OPT_RA
            s, i);
            break;
#    ifdef GC_ATOMIC_UNCOLLECTABLE
            case AUNCOLLECTABLE:
...
GC_API GC_ATTR_MALLOC
void *GC_CALL
GC_debug_generic_or_special_malloc(size_t
lb,
int knd, GC_EXTRA_PARAMS
)
{
switch (knd) {
case PTRFREE:
return
GC_debug_malloc_atomic(lb, OPT_RA
s, i);
case NORMAL:
return
GC_debug_malloc(lb, OPT_RA
...
case UNCOLLECTABLE:
return
GC_debug_malloc_uncollectable(lb, OPT_RA
s, i);
#     ifdef GC_ATOMIC_UNCOLLECTABLE
case AUNCOLLECTABLE:
...
default:
return
GC_debug_generic_malloc(lb, knd, OPT_RA
s, i);
}
}
...
(*(word *)base & 1) != 0 &&
#     endif
GC_has_other_debug_info(base)
>= 0)
return
TRUE; /* object has leaked */
...
GC_add_smashed ((ptr_t)(

&p[i])); /* alter-after-free detected */
break; /* don't report any other smashed locations in the object */
}

return
FALSE; /* GC_debug_free() has been called */
}
...
GC_debug_register_finalizer(void *obj,
