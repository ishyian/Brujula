...
STATIC ext_descr
*
GC_ext_descriptors = NULL;
/* Points to array of extended  */
/* descriptors.                 */

STATIC size_t
GC_ed_size = 0;   /* Current size of above arrays.        */
#define ED_INITIAL_SIZE 100

STATIC size_t
GC_avail_descr = 0;       /* Next available slot.         */
...
GC_add_ext_descriptor(const word *bm, word nbits) {
...
descriptor |= (descriptor & ~GC_DS_TAGS) >>
nwords;
return(descriptor);
}
...
*
GC_make_sequence_descriptor(complex_descriptor
*first,
...
STATIC ptr_t
*
GC_eobjfreelist = NULL;

STATIC mse
*
GC_typed_mark_proc(word
* addr,
mse *mark_stack_ptr,
        mse
* mark_stack_limit,
...
word *current_p = addr;
word current;
ptr_t greatest_ha = (ptr_t)
GC_greatest_plausible_heap_addr;
ptr_t least_ha = (ptr_t)
...
STATIC word
GC_descr_obj_size(complex_descriptor
*d)
{
switch(d -> TAG) {
case LEAF_TAG:
...
i<nelements;
i++) {
msp = GC_push_complex_descriptor((word * )
current, descr,
msp, msl);
if (msp == 0) return(0);
current +=
...
msp = GC_push_complex_descriptor((word * )
current, d -> sd.sd_first,
msp, msl);
if (msp == 0) return(0);
current +=
sz;
msp = GC_push_complex_descriptor((word * )
current, d -> sd.sd_second,
msp, msl);
return(msp);
}
default:
...
mse_start = (ptr_t)
addr;
new_mark_stack_ptr -> mse_descr.
w = sz | GC_DS_LENGTH;
} else {
/* Push descriptor itself */
new_mark_stack_ptr++;
new_mark_stack_ptr ->
mse_start = (ptr_t)(addr + nwords - 1);
new_mark_stack_ptr -> mse_descr.
w = sizeof(word) | GC_DS_LENGTH;
}
return
new_mark_stack_ptr;
}
...
GC_CALL GC_make_descriptor(const GC_word *bm, size_t len) {
...
op = (word * )
GC_malloc_kind(lb, GC_explicit_kind
);
if (
EXPECT(NULL
== op, FALSE))
...
GENERAL_MALLOC_IOP(lb, GC_explicit_kind);

if (0 == op) return 0;
/* See the comment in GC_malloc_explicitly_typed.   */
lg = BYTES_TO_GRANULES(GC_size(op));
} else {
GC_eobjfreelist[lg] = (ptr_t)
obj_link(op);
obj_link(op) = 0;
GC_bytes_allocd +=
GRANULES_TO_BYTES((word)
lg);
...
GC_ASSERT(GC_explicit_typing_initialized);
descr_type = GC_make_array_descriptor((word)
n, (word)lb, d, &simple_descr,
&complex_descr, &leaf);
...
case SIMPLE:
return
GC_malloc_explicitly_typed(lb, simple_descr
);
case LEAF:
lb = SIZET_SAT_ADD(lb,
...
op = (word * )
GC_malloc_kind(lb, GC_array_kind
);
if (
EXPECT(NULL
== op, FALSE))
...
ld_tag = LEAF_TAG;
lp ->
ld_size = leaf.ld_size;
lp ->
ld_nelements = leaf.ld_nelements;
lp ->
ld_descriptor = leaf.ld_descriptor;
((volatile word *)op)[
GRANULES_TO_WORDS(lg)
- 1] = (word)
lp;
...
EXPECT (GC_general_register_disappearing_link(
        (void **)(op
