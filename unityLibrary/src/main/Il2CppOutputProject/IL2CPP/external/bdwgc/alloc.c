...
GC_INNER GC_bool
GC_incremental = FALSE; /* By default, stop the world. */
#endif
...
STATIC word
GC_used_heap_size_after_full = 0;
...
GC_API void GC_CALL
GC_set_min_bytes_allocd(size_t
value)
{
GC_ASSERT(value
> 0);
...
GC_CALL GC_get_min_bytes_allocd(void) {
...
#   endif
    ...
    BZERO((word * )
    frames, CLEAR_NWORDS * sizeof(word));
}
...
GC_API void GC_CALL
GC_set_start_callback(GC_start_callback_proc
fn)
{
DCL_LOCK_STATE;
...
GC_API GC_start_callback_proc

GC_CALL GC_get_start_callback(void) {
...
#         endif
            if (GC_need_full_gc || n_partial_gcs >= GC_full_freq) {
                ...
                (void) GC_reclaim_all((GC_stop_func)
                0, TRUE);
                GC_notify_full_gc();
                GC_clear_marks();
                n_partial_gcs = 0;
...
STATIC GC_on_collection_event_proc
GC_on_collection_event = 0;

GC_API void GC_CALL
GC_set_on_collection_event(GC_on_collection_event_proc
fn)
{
...
GC_API GC_on_collection_event_proc

GC_CALL GC_get_on_collection_event(void) {
...
GC_INNER GC_bool
GC_try_to_collect_inner(GC_stop_func
stop_func)
{
#   ifndef NO_CLOCK
...
if (GC_dont_gc || (*stop_func)()) return
FALSE;
...
#   ifndef GC_GET_HEAP_USAGE_NOT_NEEDED
    if (GC_bytes_found > 0)
        GC_reclaimed_bytes_before_gc += (word)
    GC_bytes_found;
#   endif
    GC_bytes_found = 0;
...
STATIC GC_bool
GC_try_to_collect_general(GC_stop_func
stop_func,
GC_bool force_unmap
GC_ATTR_UNUSED)
{
...
GC_API int GC_CALL
GC_try_to_collect(GC_stop_func
stop_func)
{
GC_ASSERT (NONNULL_ARG_NOT_NULL(stop_func));

return (int)
GC_try_to_collect_general(stop_func, FALSE
);
}
...
void GC_print_heap_sects(void) {
    ...
    for (i = 0; i < GC_n_heap_sects; i++) {
        ...
        for (h = (struct hblk *) start; (word)h < (word)(start + len);
        h++) {
...
result = (int) GC_expand_hp_inner(divHBLKSZ((word)
bytes));
if (result) GC_requested_heapsize +=
bytes;
...
divHBLKSZ (GC_WORD_MAX))
blocks_to_get = divHBLKSZ(GC_WORD_MAX);
}

if (!
GC_expand_hp_inner(blocks_to_get)
